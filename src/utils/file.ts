...
function getStoreConfig(key: string) {
...
function getConfig(useDefault: boolean, platform: string) {
  ...
  const customConfig = getStoreConfig(`${platform}Config`)
...
async function ghFileUpload(content: string, filename: string) {
  const useDefault = getStoreConfig('imgHost') === `default`
...
async function qiniuUpload(file: File) {
  const { accessKey, secretKey, bucket, region, path, domain } = getStoreConfig('qiniuConfig')
...
async function txCOSFileUpload(file: File) {
  const dateFilename = getDateFilename(file.name)
  const { secretId, secretKey, bucket, region, path, cdnHost } = getStoreConfig(
    `txCOSConfig`,
  )
  const cos = new COS({
    SecretId: secretId,
...
async function minioFileUpload(content: string, filename: string) {
  const dateFilename = getDateFilename(filename)
  const { endpoint, port, useSSL, bucket, accessKey, secretKey } = getStoreConfig('minioConfig')
...
async function mpFileUpload(file: File) {
  const { appID, appsecret, proxyOrigin } = getStoreConfig('mpConfig')
...
async function r2Upload(file: File) {
  const { accountId, accessKey, secretKey, bucket, path, domain } = getStoreConfig('r2Config')
  const dir = path ? `${path}/` : ``
...
async function formCustomUpload(content: string, file: File) {
  const str = `
    async (CUSTOM_ARG) => {
      ${getStoreConfig('formCustomConfig')}
    }
  `
  return new Promise<string>((resolve, reject) => {
    const exportObj = {
...
function fileUpload(content: string, file: File) {
  const imgHost = getStoreConfig('imgHost')
  if (!imgHost) {
